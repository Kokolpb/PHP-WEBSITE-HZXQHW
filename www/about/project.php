...
$intRows = 12;
$strSQLNum = "SELECT COUNT(*) as num from newsinfo as a left join newspic as b on a.id_newsinfo=b.id_newsinfo where a.id_newsdir='5' and a.dele='1' and a.lang='1' order by a.id_newsinfo desc";   
...
$strSQL = "select a.id_newsinfo,a.title,a.content,a.intro,b.opicname from newsinfo as a left join newspic as b on a.id_newsinfo=b.id_newsinfo where a.id_newsdir='5' and a.dele='1' and a.lang='1' order by a.id_newsinfo desc" ;
$objDB->SelectLimit($strSQL,$intRows,$intStartNum);
$case_ny=$objDB->GetRows();
...
<div id="content_aboutrighttitle1">成功案例</div>
<div id="about_navi">您现在的位置：<A href="/">杭州鑫桥会展</A> &gt; 成功案例</div>
</div><!--end content_aboutrighttitle!-->

<div id="content_aboutrightcont">
<?php for($i=0;$i<sizeof($case_ny);$i++){  ?>
<div id="caselist">
  <a href="projectinfo.php?cid=<?php echo $case_ny[$i][id_newsinfo]?>"><img src="/upload/news/<?php echo $case_ny[$i][opicname];?>" width="160" height="150" /></a>
  <h1><a href="projectinfo.php?cid=<?php echo $case_ny[$i][id_newsinfo]?>"><?php echo $case_ny[$i][title];?></a></h1>
</div><!--end caselist!-->
<?php }?>
