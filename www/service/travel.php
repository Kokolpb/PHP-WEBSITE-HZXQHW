...
$intRows = 12;
$strSQLNum = "SELECT COUNT(*) as num from newsinfo as a left join newspic as b on a.id_newsinfo=b.id_newsinfo where a.id_newsdir='".$_GET[ndir]."' and a.dele='1' and a.lang='1' order by a.id_newsinfo desc";   
...
$strSQL = "select a.id_newsinfo,a.title,a.content,a.intro,b.opicname from newsinfo as a left join newspic as b on a.id_newsinfo=b.id_newsinfo where a.id_newsdir='".$_GET[ndir]."' and a.dele='1' and a.lang='1' order by a.id_newsinfo desc" ;
...
<div id="content_aboutrighttitle1"> <? if($_GET[ndir]==16){?>车辆服务<? }else{ ?>特色服务<? }?></div>
<div id="about_navi">您现在的位置：<A href="/">杭州鑫桥会展</A> &gt; <? if($_GET[ndir]==16){?>车辆服务<? }else{ ?>特色服务<? }?></div>
</div><!--end content_aboutrighttitle!-->

<div id="content_aboutrightcont">
<?php for($i=0;$i<sizeof($hotel_ny);$i++){  ?>
<div id="caselist">
  <a href="travelinfo.php?nid=<?php echo $hotel_ny[$i][id_newsinfo]?>&ndir=<?=$_GET[ndir];?>"><img src="/upload/news/<?php echo $hotel_ny[$i][opicname];?>" width="160" height="150" /></a>
  <h1><a href="travelinfo.php?nid=<?php echo $hotel_ny[$i][id_newsinfo]?>&ndir=<?=$_GET[ndir];?>"><?php echo $hotel_ny[$i][title];?></a></h1>
</div><!--end caselist!-->
<?php }?>
